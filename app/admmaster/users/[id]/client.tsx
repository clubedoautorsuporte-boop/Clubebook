...
import {
  ...
  Clock, User, Calendar, Trash2, TriangleAlert,
} from 'lucide-react'
...
export default function UserDetailClient({ user: initial }: { user: UserFull }) {
  ...
  async function handleDelete() {
    ...
    setDeleting(true)
    setDeleteError('')
    const res = await fetch(`/api/admmaster/users/${initial.id}`, { method: 'DELETE' })
    const data = await res.json()
    if (res.ok) {
      router.push('/admmaster/users')
    } else {
  ...
  return (
    <div className="p-6 md:p-8 max-w-5xl">
      ...
      <div className="mb-6 flex items-center justify-between">
        <Link href="/admmaster/users" className="flex items-center gap-1.5 text-[12px] text-[#4a5578] hover:text-purple-400 transition-colors">
          <ArrowLeft className="size-3.5" /> Voltar para usuários
        </Link>
        <button
          onClick={() => setShowDelete(true)}
          className="flex items-center gap-1.5 rounded-xl border border-red-500/30 bg-red-500/5 px-3 py-2 text-[12px] font-semibold text-red-400 transition hover:bg-red-500/15 hover:border-red-500/60"
        >
          <Trash2 className="size-3.5" /> Deletar usuário
        </button>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
          <div className="w-full max-w-md rounded-2xl border border-red-500/30 bg-[#0d1120] p-6 shadow-2xl">
            ...
            <div className="mb-4 rounded-xl border border-[#1a2035] bg-[#080c1a] p-3">
              <p className="text-[12px] font-semibold text-white">{initial.name ?? 'Sem nome'}</p>
              <p className="text-[11px] text-[#4a5578]">{initial.email}</p>
              <div className="mt-2 flex gap-3 text-[11px] text-[#2e3a55]">
                <span>{initial.deliveries.length} ebooks desvinculados</span>
                <span>{initial.drafts.length} rascunhos deletados</span>
              </div>
            </div>
            ...
            <div className="flex gap-2">
              <button
                ...
                disabled={deleting}
                className="flex-1 rounded-xl border border-[#1a2035] bg-[#080c1a] py-2.5 text-sm font-medium text-[#4a5578] hover:text-white transition disabled:opacity-40"
              >
                Cancelar
              </button>
              <button
              ...
              >
                {deleting
                  ...
                  : <><Trash2 className="size-4" /> Deletar permanentemente</>
                }
              </button>
            </div>
          </div>
